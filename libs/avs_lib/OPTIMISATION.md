...
The original AVS had a three-phase SMP interface in its base effect class (`r_defs.h`):
...
Thread management (`r_list.cpp`) used a persistent thread pool with Windows Events:
