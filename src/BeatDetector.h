...
//
// BeatDetector - Audio beat detection for visualization sync
//
// ARCHITECTURE NOTE:
// Beat detection lives in ofxAVS (not avs_lib) because:
// 1. Modern onset detection needs raw FFT magnitudes (only available here)
// 2. avs_lib AudioData has already been processed (log/dB scaled)
// 3. Different hosts may want different beat detection algorithms
// 4. avs_lib stays pure rendering, host provides isBeat
//
// This file contains the classic AVS algorithm (energy-based) plus
// infrastructure for modern spectral flux detection.

#pragma once

#include "core/configurable.h"
#include "core/effect_base.h"  // For AudioData typedef
#include <cstdint>
#include <cstring>
#include <algorithm>
...
public:
    ...
    // Modern mode: uses raw FFT magnitudes for spectral flux detection
    // Call this instead of process() when raw FFT data is available
    // Parameters: left/right amplitude arrays from FFT, bin count
    bool processModern(const float* amplitudeLeft, const float* amplitudeRight, int binSize);
    ...
    const avs::EffectUILayout& get_ui_layout() const override { return ui_layout_; }
    avs::ParameterGroup& parameters() override { return parameters_; }
    const avs::ParameterGroup& parameters() const override { return parameters_; }

private:
    avs::ParameterGroup parameters_;
    static const avs::EffectUILayout ui_layout_;

    // Stage 1: Energy detection state (classic mode)
    int beat_peak1_ = 0;
    int beat_peak2_ = 0;
    ...
    // Modern mode: spectral flux state
    static constexpr int FLUX_HISTORY_SIZE = 43;  // ~0.7s at 60fps
    float prev_spectrum_left_[1024] = {0};
    float prev_spectrum_right_[1024] = {0};
    float flux_history_[FLUX_HISTORY_SIZE] = {0};
    int flux_history_pos_ = 0;
    int flux_history_count_ = 0;

    // Internal methods - classic mode
    int detectRawBeat(const avs::AudioData& visdata);
    ...
    // Internal methods - modern mode
    float calcSpectralFlux(const float* amplitude, float* prevSpectrum, int binSize);
    float calcAdaptiveThreshold();
